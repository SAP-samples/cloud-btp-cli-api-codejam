...
👉 Now add a line to the end, which defines the alias - if you're typing it in manually, first, well done, and second, make sure you use the command as shown here, with the single quotes (inside the double quotes) and using the *append* redirection which is a double greater-than sign `>>`, and *not* a single one:
